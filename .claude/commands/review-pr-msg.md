---
description: Review PR message as maintainer before submission
allowed-tools: Read, Grep, Bash(git:*)
argument-hint: [pr-message-file-path]
---

# Task: Review PR Message as Maintainer

**PR message file:** $ARGUMENTS (default: PR_MSG.md)

**Purpose:** Review a PR message draft from a maintainer's perspective to identify issues before submission.
...
You are evaluating whether to merge this PR into your codebase. You have NOT worked on this code.
...
```bash
# Verify commit count
CLAIMED_COMMITS=$(grep -i "commits" $ARGUMENTS | grep -oE '[0-9]+' | head -1)
ACTUAL_COMMITS=$(git log --oneline main..HEAD | wc -l)
echo "Claimed: $CLAIMED_COMMITS, Actual: $ACTUAL_COMMITS"
...
**Foundation:** Context (PR + git history), Model (critical evaluation), Prompt (maintainer perspective), Tools (git + verification commands).
